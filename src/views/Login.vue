  ...
  <div class="login-page">
    ...
    <div class="login-card" :class="{ 'login-card--shake': shaking }">
      ...
      <div class="login-heading">
        ...
        <p class="login-desc">
          {{ step === 1 ? 'Enter email to get OTP' : 'Enter OTP sent to your email' }}
        </p>
      </div>
      ...
      <div class="input-group" :class="{ 'input-group--error': error, 'input-group--success': success }">
        ...
        <div class="input-wrap">
          ...
          <input ref="inputRef" v-model="inputValue" :type="step === 1 ? 'email' : 'text'" class="passcode-input"
            :placeholder="step === 1 ? 'Enter email' : 'Enter OTP'" autocomplete="current-password" maxlength="40"
            @keydown.enter="handleLogin" @input="error = ''" />
            <div class="toggle-btn"  v-if="step !== 2">
            <button  type="button" class="toggle-btn" @click="showPasscode = !showPasscode" tabindex="-1">
              <svg v-if="!showPasscode" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                stroke-width="2">
          ...
          <button v-if="step === 2" @click="resetToEmail" class="toggle-btn">
            Change email
          </button>
        </div>
        <div v-if="step === 2" class="otp-actions">
          <button
            class="resend-btn"
            @click="resendOtp"
      ...
      <!-- Submit -->
      <button class="login-btn" :class="{ 'login-btn--loading': loading, 'login-btn--success': success }"
        @click="handleLogin" :disabled="loading || !inputValue || (step === 2 && inputValue.length < 4)">
      ...
      <div class="side-panel__inner">
        ...
        <div class="side-deco">
          <svg viewBox="0 0 200 200" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path
              d="M100 10 C100 10 110 40 100 60 C90 80 70 85 70 100 C70 120 85 135 100 140 C100 140 80 155 80 175 C80 190 90 200 100 200"
              stroke="rgba(139,111,71,0.3)" stroke-width="1.5" />
            <path d="M100 10 C100 10 90 40 100 60 C110 80 130 85 130 100 C130 120 115 135 100 140"
              stroke="rgba(139,111,71,0.3)" stroke-width="1.5" />
...
const API_URL = 'https://script.google.com/macros/s/AKfycbzpATipqgEGTMVhcL_1VoaG6rrx6w69q7D4Yxhv1LHvpYQN-URCxzVcOsGtzRl6GNjRmg/exec'
...
async function sendOtp() {
  ...
  contact.value = inputValue.value
  ...
  try {
    ...
    if (data.success) {
      step.value = 2
      inputValue.value = ''
      error.value = `OTP sent to ${contact.value}`
...
.input-group--error .passcode-input {
  border-color: #e05252;
  box-shadow: 0 0 0 4px rgba(224, 82, 82, 0.08);
}

.input-group--success .passcode-input {
  border-color: #3d9e6e;
  box-shadow: 0 0 0 4px rgba(61, 158, 110, 0.1);
}
...
.login-btn--success {
  background: linear-gradient(135deg, #2e9e6e, #3dc48a);
