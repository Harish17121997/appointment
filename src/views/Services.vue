...
// Allow external pre-fill (from BookingModal "Create Bill" button via route query or prop)
function prefillBill({ name, phone, services }) {
  bill.customerName = name || ''
  bill.customerPhone = phone || ''
  // Try to match service names to prices
  if (services) {
    const svcNames = services.split(',').map(s => s.trim())
    svcNames.forEach(svcName => {
      let price = 0
      for (const cat of allServicesFlat) {
        const found = cat.services.find(s => s.name.toLowerCase() === svcName.toLowerCase())
        if (found) { price = found.price; break }
      }
      if (svcName) {
        bill.items.push({ serviceName: svcName, staff: '', rate: price, qty: 1 })
      }
    })
  }
